...
def test_repo_changelog_matches_current_project_version():
    version = read_project_version(REPO_ROOT / "pyproject.toml")

    validate_changelog(REPO_ROOT / "CHANGELOG.md", version)


def test_validate_changelog_accepts_matching_version_with_bullets(tmp_path):
...
def test_validate_changelog_rejects_missing_version_heading(tmp_path):
    changelog_path = tmp_path / "CHANGELOG.md"
    changelog_path.write_text("# Changelog\n\n## 1.2.2\n\n- Previous release.\n")
...
def test_validate_changelog_rejects_empty_version_section(tmp_path):
    changelog_path = tmp_path / "CHANGELOG.md"
    changelog_path.write_text("# Changelog\n\n## 1.2.3\n\nParagraph only.\n")
...
def test_release_branch_check_rejects_non_main():
