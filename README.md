...
- applying optional target-supplied font settings plus line, spine, and tick settings
...
with prepare_figure(fig, hide_grid=True, font_family="Aptos") as fig_export:
...
save_fig(fig, "plot.png", width=4, height=3, dpi=200, font_family="Aptos")
```

When `font_family` is omitted, `pubify-mpl` does not force figure text to a
target-specific family. Downstream packages such as `pubify-tex` and
`pubify-ppt` own their own typography profiles.
